---
author: kecsi
categories:
- linux
created: 1206005591
date: '2008-03-20T00:00:00Z'
description: 'Escputil egy parancsori alkalmazás Epson nyomtatók kezeléséhez. Néhány példa mire lehet használni. Pl. tinta szint lekérdezés escputil -i -u -r /dev/usb/lp0 tisztítás: escputil -c -u -r /dev/usb/lp0 fúvóka ellenőrzés: escputil -n -u -r /dev/usb/lp0 printer fej beállítás: escutil -a -u -r /dev/usb/lp0 színes printer fej beállítás: escputil -o -u -r /dev/usb/lp0'
title: Epson CLI utasítások
aliases:
- /node/483/
- /story/483/
---
Escputil egy parancsori alkalmazás Epson nyomtatók kezeléséhez.

Néhány példa mire lehet használni.
Pl. tinta szint lekérdezés
<code>escputil -i -u -r /dev/usb/lp0</code>
tisztítás:
<code>escputil -c -u -r /dev/usb/lp0</code>
fúvóka ellenőrzés:
<code>escputil -n -u -r /dev/usb/lp0</code>
printer fej beállítás:
<code>escutil -a -u -r /dev/usb/lp0</code>
színes printer fej beállítás:
<code>escputil -o -u -r /dev/usb/lp0</code>
