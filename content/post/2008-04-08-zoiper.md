---
author: kecsi
categories:
- linux
created: 1207640258
date: '2008-04-08T00:00:00Z'
description: Zoiper egy VoIP szoftveres kliens, együttműködik Asterisk és SIP kompatibilis IP-alapú kommunkációs rendszerekkel. Letölthető egy ingyenes verzió is. A Zoiper 2.0 IAX & SIP szoftveres (ip)telefon szoftver (korábban Idefisk néven ismert) már támogatja a T.38 IP Faxot is. Zoiper elérhető Windowsra, Mac OS X-re és Linuxra.
title: Zoiper
aliases:
- /node/486/
- /story/486/
---
<a href="http://www.zoiper.com">Zoiper</a> egy VoIP szoftveres kliens, együttműködik Asterisk és SIP kompatibilis IP-alapú kommunkációs rendszerekkel. Letölthető egy ingyenes verzió is. A Zoiper 2.0 IAX & SIP szoftveres (ip)telefon szoftver (korábban Idefisk néven ismert) már támogatja a T.38 IP Faxot is. 
Zoiper elérhető Windowsra, Mac OS X-re és Linuxra.
