...
description: Hosszú évek után a múlt héten találkoztam újra azzal a problémával, hogy egy macintos rendszerben fejlesztett kódot, amihez egy windows-os környezetben is hozzányúltak, kellett bevezetnem verziókezelőbe. 2013 ide vagy oda, a sorvégek még mindíg gondot tudnak okozni a közös munka folyamán.
tags:
- dos2unix
- unix2dos
- tofrodos
 ...
 * `find . -type f`: az aktuális könyvtárban állva kilistázza az összes alkönyvtár összes állományát, kivéve a könyvtárakat
 * `-regextype posix-awk -regex "(.*.php)"`: ezek közül mintaillesztéssel csak a .php kiterjesztésű állományok kiválogatása
 * `-exec sed -i s/\\r//g`: carriage return cseréje sed-el
 * `{} ';'`: a find parancs által előállt állománylista paraméterként való átadása a sed-nek.
