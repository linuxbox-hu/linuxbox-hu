---
author: kecsi
categories:
- x
created: 1166780629
date: '2006-12-22T00:00:00Z'
description: TunaPie egy Internet rádió és TV stream böngésző. Teoretikusan bármely linux rendszeren működik, de csak linuxon és Max OS X-en tesztelték. 0.9.6-os verziótól kompatibilis az Icecast protokollal is valamint támogatja a Shoutcast (winamp) streamek is. Tunapie-vel megkeresed a hallgatni-nézni valót majd innét indíthatod a kedvenc zene vagy video lejátszód. Természetesen a források mentése is megengedett. Ubuntu csomag még csak feisty-ben létezik ha jól látom.
title: Tunapie
aliases:
- /node/261/
- /story/261/
---
<a href="http://tunapie.sourceforge.net/">TunaPie</a> egy Internet rádió és TV stream böngésző. Teoretikusan bármely linux rendszeren működik, de csak linuxon és Max OS X-en tesztelték. 0.9.6-os verziótól kompatibilis az Icecast protokollal is valamint támogatja a Shoutcast (winamp) streamek is. Tunapie-vel megkeresed a hallgatni-nézni valót majd innét indíthatod a kedvenc zene vagy video lejátszód. Természetesen a források mentése is megengedett.

Ubuntu csomag még csak feisty-ben létezik ha jól látom.
